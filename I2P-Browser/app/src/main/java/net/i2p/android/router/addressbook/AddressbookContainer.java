...
public class AddressbookContainer extends Fragment
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (!mTwoPane) {
            ...
            TitlePageIndicator pageIndicator = v.findViewById(R.id.page_indicator);
            mFragPagerAdapter = new AddressbookPagerAdapter(getActivity(), getChildFragmentManager());
            mViewPager.setAdapter(mFragPagerAdapter);
            pageIndicator.setViewPager(mViewPager);
        }

        if(!status.isOpenURL && mViewPager!=null){
    ...
    public void onAddressSelected(CharSequence host) {
        if (Intent.ACTION_PICK.equals(getActivity().getIntent().getAction())) {
            ...
            if(status.isProxyRunning()){
                Intent i = new Intent(Intent.ACTION_VIEW);
                i.setData(Uri.parse("http://" + host));
                activityContextManager.getInstance().getHomeController().onLoadURL(Uri.parse("http://" + host).toString(),false);
                getActivity().finish();
            }else {
                pluginController.getInstance().MessageManagerHandlerI2P(getActivity(), enums.etype.on_proxy_loading);
            }
        }
    }
